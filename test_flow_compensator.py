...
from flow_compensator import FlowCompensator, GCodeParser
...
"""
    ...
    def test_parse_metadata_regex(self):
        """Test metadata extraction from G-code with regex parser."""
        parser = GCodeParser(Path(self.temp_file.name), parser_mode="regex")
    ...
    def test_parse_metadata_library(self):
        """Test metadata extraction from G-code with library parser."""
        parser = GCodeParser(Path(self.temp_file.name), parser_mode="library")
        metadata = parser.parse_metadata()
    ...
    def test_parse_move_with_extrusion_regex(self):
        """Test parsing G1 move with extrusion using regex."""
        parser = GCodeParser(Path(self.temp_file.name), parser_mode="regex")
    ...
    def test_parse_move_with_extrusion_library(self):
        """Test parsing G1 move with extrusion using library."""
        parser = GCodeParser(Path(self.temp_file.name), parser_mode="library")
        parser.read_all_lines()
        ...
        self.assertIsNotNone(move_info)
        self.assertEqual(move_info["extrusion"], 5.55)
        self.assertEqual(move_info["distance"], 10.0)
        self.assertEqual(move_info["feedrate"], 3000)
        self.assertEqual(new_pos["x"], 110)
        self.assertEqual(new_pos["y"], 100)

    def test_parse_travel_move_regex(self):
        """Test parsing travel move without extrusion using regex."""
        parser = GCodeParser(Path(self.temp_file.name), parser_mode="regex")
...
class TestMultiMaterialSupport(unittest.TestCase):
    ...
    def test_load_extruder_mapping(self):
        ...
        self.config["extruder_mapping"] = {"T0": "PETG", "T1": "PVA"}
